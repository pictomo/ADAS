# ARCデータセットから評価用データをサンプリングし、検証セットとテストセットに分割するスクリプト
# グリッドサイズが小さい（5x5以下）タスクのみを対象とし、pickleファイルとして保存する
...
# グリッドサイズのフィルタリング：入力・出力ともに5x5以下のタスクのみを抽出
...
for arc_file in arc_files:
    ...
    if (
        "train" in arc_data
        and len(arc_data["train"]) > 0
        and "input" in arc_data["train"][0]
    ):
